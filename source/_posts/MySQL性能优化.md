...
1. read uncommitted
2. read committed
普通的select使用快照读，底层是MVCC实现的。加锁的select都使用的是记录锁。
3. repeatable read
普通的select使用快照读（snapshot read），底层使用MVCC来实现的。加锁的select以及更新操作使用当前读（current read），底层使用的记录锁，间隙锁，临键锁
4. serialiable

#### MVCC的原理
如果要让一个事务前后两次读取的数据保持一致，那么我们在修改数据的时候给它建立一个备份或者叫快照，然后在来去这个快照就行了。这叫做多版本控制Muti Version Concurrency Control（MVCC）
MVCC的核心思想是：我可以查到我这个事务开始之前的数据，即使它在后面被修改或删除了。在我这个事务之后的数据，我是查不到的。

实现原理：
InnoDB为每行记录都实现了两个隐藏字段
DB_TRX_ID，6字节：插入或更新行的最后一个事务的事务ID，事务编号是自动递增的（可以理解为版本号）
DB_ROLL_PTR，7字节：回滚指针（可以理解为删除版本号）

MVCC查找规则：只能查找创建时间小于等于当前事务ID的数据，和删除大于当前事务ID的行
在InnoDB中，MVCC是通过Undo log实现的。

#### 锁的分类，行锁的原理，行锁的算法
官网把InnoDB的锁分成8类。我们把前面两个行级别的锁（shared and Exclusive Locks），和两个表级别锁（Intention Locks）称为锁的基本模式。
后面三个Record Locks，Gap Locks， Next-Key Locks，叫做锁的算法，也就是分别在什么情况下锁定什么范围。

锁的粒度：
表锁：锁的粒度大，锁的效率高，锁冲突的概率大，并发性低
行锁：锁的粒度小，锁的效率低，锁冲突的概率小，并发性高

共享锁（Shared Locks）：获取了一行数据的读锁后，可以用来读取数据，所以它也叫读锁。注意不要在加上读锁之后写数据，不然可能出现死锁。多个事务可以共享一把读锁。可以使用select ... lock in share mode；的方式加上一把读锁。
排它锁（Exclusive Locks）：用来操作数据的，也叫做写锁。只要一个事务获取了一行数据的排它锁，其他的事务就不能获取这一行数据的共享锁和排它锁。排它锁加锁有两种方式，一种是自动加排它锁，我们在操作数据的时候，包括增删改，都会默认加一个排它锁。一种是手工用一个for update给一行数据加上一个排它锁。
意向锁：由数据库自己维护的，当我们给一行数据加上共享锁之前，数据库会自动在这张表上加一个意向共享锁，当我们给一行数据加上排它锁之前，数据库会自动在这张表上加一个意向排它锁。

InnoDB的行锁，是通过锁住索引来实现的。
如果我们定义了主键（primary key），那么InnoDB会选择主键作为聚集索引。
如果没有显式定义主键，则InnoDB会选择第一个不包含 NULL 值的唯一索引作为主键索引。
如果也没有这样的唯一索引，InnoDB会选择内置一个6个字节的 rowid 作为隐藏的聚集索引，它会随着行记录的写入而主键递增。

死锁的避免：
1. 在程序中，操作多张表时，尽量以相同的顺序来访问（避免形成等待环路）
2. 批量操作单张表数据时，先对数据进行排序避免形成等待环路）
3. 申请足够级别的锁，如果是操作数据，就申请排它锁。
