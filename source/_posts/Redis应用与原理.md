...
##### RDB
RDB是Redis默认的持久化方案。当满足一定的条件的时候，会把当前内存中的数据写入磁盘，生成一个快照文件dump.rdb。Redis重启会通过加载dump.rdb文件恢复数据
Redis.conf，SNAPSHOTTING，其中定义了触发把数据保存到磁盘的触发频率，如果不需要RDB方案，注释save或配置成空字符串“”,只要满足一个条件就会触发。
```property
...
```
**优点**
1. RDB是一个非常紧凑（compact）的文件，它保存了redis在某个时间点上的数据集，这种文件非常适合进行备份和灾难恢复。
2. 生成RDB文件的时候，redis主进程会fork一个子进程来处理所有的保存工作，主进程不用进行任何IO操作。
3. RDB在数据恢复的时候比AOF恢复的速度要快。

**缺点**
1. RDB方式数据没办法做到实时持久化/秒级持久化。因为bgsave每次运行都要执行fork操作创建子进程，频繁执行成本过高。
2. 在一定间隔时间做一个备份，所以redis如果意外down掉，就会丢失最后一次快照之后的数据。

##### AOF
Redis默认不开启，AOF采用日志的形式来记录每次的写操作，并追加到文件中。开启后，执行更改redis数据命令时，就会把命令写如到AOF文件中。
